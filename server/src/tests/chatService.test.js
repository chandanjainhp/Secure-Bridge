...
const mockMcpTools = jest.fn();

jest.unstable_mockModule("ai", () => ({
  generateText: jest.fn(),
}));

jest.unstable_mockModule("../services/mcpClient.js", () => ({
  listMcpToolsForAiSdk: mockMcpTools,
}));

jest.unstable_mockModule("../utils/chat.providers.js", () => ({
...
jest.unstable_mockModule("../features/usage/services/usageService.js", () => ({
  default: {
    incrementUsage: jest.fn(() => Promise.resolve()),
    checkAndIncrementUsage: jest.fn(() => Promise.resolve()),
...
const { listMcpToolsForAiSdk } = await import("../services/mcpClient.js");
const { generateText } = await import("ai");
const { getModelInstance, detectProvider, findExternalKeys } =
...
describe("ChatService", () => {
  ...
  beforeEach(() => {
    jest.clearAllMocks();
    listMcpToolsForAiSdk.mockResolvedValue({});
  });
  ...
  describe("buildMessagesWithSystemPrompt", () => {
    ...
    test("skips empty system prompt", () => {
      const messages = buildMessagesWithSystemPrompt("   ", [], "Hello");
  ...
  describe("MCP tool integration", () => {
    test("passes MCP tools to generateText when the MCP server is enabled", async () => {
      detectProvider.mockReturnValueOnce({
        provider: "openai",
        model: "gpt-4o-mini",
      });
      getModelInstance.mockResolvedValueOnce({
      ...
      listMcpToolsForAiSdk.mockResolvedValueOnce({
        fetch_url: {
          ...
          execute: jest.fn(async ({ url }) => ({ ok: true, url })),
        },
      });
      ...
      await generateChatResponse({
        ...
        conversationHistory: [],
        userContent: "Check this URL",
        model: "gpt-4o-mini",
        temperature: 0.7,
        maxTokens: 256,
      });

      expect(listMcpToolsForAiSdk).toHaveBeenCalled();
      expect(generateText).toHaveBeenCalledWith(
        expect.objectContaining({
          tools: expect.objectContaining({
            fetch_url: expect.objectContaining({
