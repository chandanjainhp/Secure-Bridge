...
class ApiKeyController {
  ...
  static getAllApiKeys = asyncHandler(async (req, res) => {
    ...
    return res.status(200).json(
      new ApiResponse(200, result, "API keys retrieved successfully")
    );
  });
  ...
  static getApiKeyDetails = asyncHandler(async (req, res) => {
    ...
    const apiKeyDetails = await ApiKeyService.getApiKeyDetails(keyId, req.user._id);
    
    return res.status(200).json(
      new ApiResponse(200, apiKeyDetails, "API key details retrieved successfully")
    );
  });
  ...
  static createApiKey = asyncHandler(async (req, res) => {
    ...
    const apiKey = await ApiKeyService.createApiKey(req.user._id, sanitizedData);
  ...
  static updateApiKey = asyncHandler(async (req, res) => {
    ...
    if (updateData.description !== undefined) {
      updateData.description = sanitizers.sanitizeDescription(updateData.description);
    }
    if (updateData.ipWhitelist) {
      updateData.ipWhitelist = sanitizers.sanitizeIpList(updateData.ipWhitelist);
    }
    if (updateData.domainWhitelist) {
      updateData.domainWhitelist = sanitizers.sanitizeDomainList(updateData.domainWhitelist);
    }
    ...
    return res.status(200).json(
      new ApiResponse(200, response, "API key updated successfully")
    );
  });
  ...
  static regenerateApiKey = asyncHandler(async (req, res) => {
    ...
    const regeneratedKey = await ApiKeyService.regenerateApiKey(keyId, req.user._id, auditInfo);
    
    return res.status(200).json(
      new ApiResponse(200, regeneratedKey, "API key regenerated successfully")
    );
  });
  ...
  static revokeApiKey = asyncHandler(async (req, res) => {
    ...
    const result = await ApiKeyService.revokeApiKey(keyId, req.user._id, auditInfo);
    
    return res.status(200).json(
      new ApiResponse(200, result, "API key revoked successfully")
    );
  });
  ...
  static testApiKey = asyncHandler(async (req, res) => {
    ...
    const testResult = await ApiKeyService.testApiKey(keyId, req.user._id, { testType });
  ...
  static getUsageAnalytics = asyncHandler(async (req, res) => {
    ...
    const analytics = await ApiKeyService.getUsageAnalytics(keyId, req.user._id, options);
    
    return res.status(200).json(
      new ApiResponse(200, analytics, "Usage analytics retrieved successfully")
    );
  });
  ...
  static validateExternalKeyFormat = asyncHandler(async (req, res) => {
    ...
    return res.status(200).json(
      new ApiResponse(200, validation, "API key validation completed")
    );
  });
  ...
  static testExternalKeyConnectivity = asyncHandler(async (req, res) => {
    ...
    const formatValidation = await ApiKeyController.validateExternalKeyFormat(req, res);
    ...
    return res.status(200).json(
      new ApiResponse(200, testResult, "External API key connectivity test completed")
    );
  });
  ...
  static getUsageSummary = asyncHandler(async (req, res) => {
    ...
    const summary = {
      ...
      totalRequests: activeKeys.reduce((sum, key) => sum + key.usage.totalRequests, 0),
      keysNearLimit: 0,
      keysByProvider: {},
      recentActivity: []
    };
    ...
    activeKeys.forEach(key => {
      ...
      if (dailyRequests / dailyLimit > 0.8) { // 80% threshold
        summary.keysNearLimit++;
      }

      // Count by provider
      const provider = key.externalProvider || 'internal';
      summary.keysByProvider[provider] = (summary.keysByProvider[provider] || 0) + 1;
    });
  ...
  static exportApiKeys = asyncHandler(async (req, res) => {
    ...
    if (format === 'csv') {
      ...
      res.setHeader('Content-Disposition', 'attachment; filename="api-keys-export.csv"');
      return res.send(csv);
    }

    res.setHeader('Content-Type', 'application/json');
    res.setHeader('Content-Disposition', 'attachment; filename="api-keys-export.json"');
  ...
  static async _testAzureConnectivity(apiKey) {
    ...
    return {
      success: true,
      message: 'Azure OpenAI API key format is valid (endpoint-specific testing requires configuration)',
      details: {
